...
public class WebSecurityConfig {
    ...
    public SecurityFilterChain configure(HttpSecurity security) throws Exception{
        ...
        security
        // Basic 인증 방식에 대한 설정
        // Basic 인증 방식 미사용으로 지정 --> 우리는 Bearer인증 방식을 쓰기에
        ...
        //! 선생님이 중요하데 authorizeHttpRequests 이 파트
        // 인가 작업
        ...
        .authorizeHttpRequests(request -> request
            // requestMatchers(): URL 패턴, HTTP 메서드, URL 패턴 + HTTP메서드 마다 접근 권한을 부여하는 메서드
            // permitAll() : 모든 클라이언트가 접근할 수 있도록 지정
            // authenticated() : 인증된 모든 클라이언트가 접근할 수 있도록 지정
            // hasRole(권한) : 특정 권한을 가진 클라이언트만 접근할 수 있도록 지정 , 매개변수로 전달되는 권한명은 ROLE_를 제거한 실제 권한명
            .requestMatchers("/basic", "/basic/**", "/security", "/security/**", "/oauth/**").permitAll()
