...
// @AllArgsConstructor
// 만약 의존성을 선택적으로 주입하겠다한다면 의존성을 final로 지정하여 @RequiredArgsConstructor를 이용할 수 있다.
@RequiredArgsConstructor
public class ServiceController {

    // 의존성 주입 (DI) :
    // - 해당 모듈에 필요한 의존성을 클래스의 인스턴스를 생성하는 위치에서 주입
    // - Spring framework에서 @Autowired 을 이용하여 의존성 주입을 Spring에게 위임할 수 있다 (IOC, DI)
    // - 필드를 이용한 의존성 주입, setter 메서드를 이용한 의존성 주입, 생성자를 이용한 의존성 주입
    // - 생성자를 이용한 의존성 주입 방법이 안정적이므로 Spring Framework에서는 생성자를 이용한 의존성 주입을 권장
