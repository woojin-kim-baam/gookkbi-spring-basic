...
@Entity(name = "sample_table_1") // 애는 자바에서 쓸때 이름

// @Table : 
// - 해당 Entity 클래스를 RDBMS의 테이블과 매핑하는 어노테이션
// - 만약 java의 클래스명과 RDBMS의 테이블명이 동일하다면 유추하여 매핑함
// - name 속성 : 클래스명과 테이블명이 서로 다를 때 테이블명을 직접 명시하는 속성
@Table(name = "sample_table_1") // 애는 DBMS에서 쓸 때의 이름
...
public class SampleTable1Entity {
    ...
    // - JPA에서는 @ID로 지정된 멤버 변수가 반드시 하나 이상 존재해야 함
    @Id
...
// @ManyToOne, @OneToMany, @OntToOne,... 관계를 위한 어노테이션도 존재함
// 관계 관련 어노테이션을 사용하면 성능이 기하급수적으로 떨어짐

// application.properties에 spring.jpa.hibernate.ddl-auto 라는 속성으로 Entity 클래스에 해당하는 테이블을 DBMS에 자동으로 생성되게 할 수 있음음
