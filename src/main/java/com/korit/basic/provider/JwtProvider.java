...
// - 암호화가 되어 있어 클라이언트와 서버간에 안전한 데이터 전송을 수행할 수 있음
// - 헤더 : 토큰의 유형, 서명에 사용된 암호화 알고리즘이 지정되어 있음
...
public class JwtProvider {
    ...
    // 2. 시스템의 환경변수로 등록하여 사용(윈도우키 눌러서 시스템 환경변수 편집 검색)
    // - OS 자체의 시스템 환경변수에 비밀키를 등록
    // - Spring에서 시스템 환경변수를 읽어서 사용

    // 3. 외부 데이터 관리 도구를 사용
    // - 자체 서버가 아닌 타 서버에 등록된 Vault도구를 사용하여 비밀키를 관리
    // - OS 부팅시에 Vault 서버에서 비밀키를 가져와 사용
    // - OS 부팅 시 마다 새로운 비밀키를 자동으로 부여함
