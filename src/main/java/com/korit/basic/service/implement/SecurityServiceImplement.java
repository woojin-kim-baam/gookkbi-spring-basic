...
public class SecurityServiceImplement implements SecurityService {
    ...
    // - 비밀번호를 쉽고 안전하게 암호하하여 관리할 수 있도록 도움을 주는 인터페이스
    // - 구현체 : BCryptPasswordEncoder, SCryptPasswordEncoder, Pbkdf2PasswordEncoder
    // - String encode(평문비밀번호) : 평문 비밀번호를 암호화하여 반환
    // - boolean matches(평문 비밀번호, 암호화된비밀번호) : 평문비밀번호와 암호화된 비밀번호가 서로 일치하는지 확인
    private PasswordEncoder passwordEncoder = new BCryptPasswordEncoder(); // 내부 주입 이유 : 상태마다 바뀔 이유가 없기에 가능하다.
